...
if [ -z "$1" ] || [ -z "$2" ] || [ -z "$3" ]
then
   echo "Usage: assign-guaranteed-mem-limit.sh <deployment_name> <namespace> <mem_limit>"
   echo "Error: Please enter deployment name and namespace and memory limit (either in bytes or the equivalent suffixes (i.e. 128974848=129e6=129M=123Mi)"
   exit 1
fi

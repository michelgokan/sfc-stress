...
if [ -z "$1" ] || [ -z "$2" ] || [ -z "$3" ]
then
   echo "Usage: assign-guaranteed-cpu-limit-to-statefulset.sh <statfulset_name> <namespace> <cpu_limit>"
   echo "Error: Please specify statefulset name and namespace and cpu limit (eaither an integer or the millicore (i.e. 1000m=1)"
   exit 1
fi

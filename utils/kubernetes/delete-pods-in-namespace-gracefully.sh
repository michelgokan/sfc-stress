...
kube_result=$($ROOTPATH/bin/utilities/kubernetes/get-pods-in-namespace.sh $1 | jq '[.items[] | {n: .metadata.name, nn: .metadata.namespace}]' | jq '.[] | "kubectl delete pod \(.n) --namespace \(.nn)"' -r )
